...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("laliga")
    .setDescription("List open LaLiga matches"),
  async execute(interaction) {
    ...
    if (matches.length === 0) {
      const embed = buildEmbed({
        title: "No open LaLiga matches ⚽",
        description: "No LaLiga games are open for betting right now.",
        color: 0x6ae4c5
      });
    ...
    const embeds = descriptions.map((description, index) => buildEmbed({
      title: descriptions.length > 1
        ? `Open LaLiga matches ⚽ (${index + 1}/${descriptions.length})`
        : "Open LaLiga matches ⚽",
      description,
      color: 0xf6c244
    }));
